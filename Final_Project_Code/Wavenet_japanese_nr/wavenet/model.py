...
# from .testCMG_v2 import get_mixture_coef, temp_control, get_lossfunc
# from .testCMG_v3 import get_mixture_coef, temp_control, get_lossfunc
from .testCMG_v2_sharada import get_mixture_coef, temp_control, nll_loss
import pdb
...
class WaveNetModel(object):
    ...
    def __init__(self,
        ...
        self.CMG_channels = 4 * AP_channels   # each output dimention will be modeled by 4 paras of CMGs
        ...
        with tf.variable_scope('wavenet'):
            ...
            with tf.variable_scope('causal_layer'):
                ...
                if self.scalar_input:
                    ...
                    initial_channels = self.AP_channels + self.MFSC_channels
                    initial_filter_width = self.initial_filter_width
                ...
                # layer["filter_bias"] = create_variable(
                #     'filter_bias',
                ...
                for i, dilation in enumerate(self.dilations):  # dilations has 5 elements
                    with tf.variable_scope('layer{}'.format(i)):
                        ...
                        if self.use_biases:
                            ...
                            # current["lc_gate_bias"] = create_variable(
                            #     "lc_gate",
                            #     [self.dilation_channels])
                            # current["lc_filt_bias"] = create_variable(
                            #     "lc_filt",
                            #     [self.dilation_channels])
            ...
            with tf.variable_scope('postprocessing'):
                ...
                current["CMG_paras"] = create_variable(
                    "CMG_paras",
                    [1, self.skip_channels, self.CMG_channels])     ################# might have the issue of (60, ) ########################
                if self.use_biases:
    ...
    def _create_causal_layer(self, input_batch):
        ...
        with tf.name_scope('causal_layer'):
            ...
            return causal_conv(input_batch, weights_filter, 1) #+ self.variables["causal_layer"]["filter_bias"]       # dilation = 1 
    ...
    def _create_network(self, input_batch, 
        ...
        # return input_batch.get_shape(), \
        #         F0_batch_dilated.get_shape(), \
        #         prev_phone_batch_dilated.get_shape(), \
        #         cur_phone_batch_dilated.get_shape(), \
        #         next_phone_batch_dilated.get_shape(), \
        #         phone_pos_batch_dilated.get_shape() 
    ...
    def predict_proba(self, waveform, ap_channels, lc_batch, global_condition=None, name='wavenet'):
        ...
        with tf.name_scope(name):
            ...
            tau = np.array([0.001] * self.AP_channels)
            tau = tf.constant(tau, dtype=tf.float32)

            # print (prediction.get_shape())
            # print (target_output.get_shape())
    ...
    def loss(self,
        ...
        with tf.name_scope(name):
            ...
            ########################## ###############################
            ######################### no longer need this mu_law_encode, now the input_batch is alreay (m, 60)
            # encoded_input = mu_law_encode(input_batch,               # input_batch is shape [receptive_field + sample_size, 1], see train.py
            #                               self.quantization_channels)
            
            # input_batch = tf.reshape(input_batch, [1, input_batch.shape[-2], input_batch.shape[-1]])    # validate shape
            # lc_batch = tf.reshape(lc_batch, [1, lc_batch.shape[-2], lc_batch.shape[-1]])
            ...
            # encoded = self._one_hot(encoded_input)   # encoded audio shape: [batch_size(1), receptive_field + sample_size, quantization_channels]
            if self.scalar_input:
            ...
            # Cut off the first sample of network control input to preserve causality.
            # lc_input = tf.slice(lc_batch, [0, 1, 0],[-1, -1, -1])           ######################################################
            ...
            with tf.name_scope('loss'):
                ...
                target_output = tf.reshape(input_batch,
                        [self.batch_size, -1, self.AP_channels + self.MFSC_channels])
                target_output = target_output[:,:,:4]     # take first 4 colums because they corresponds to ap
                ...
                #                         np.array([0.5] * 51)),
                #                         axis = 0)
                tau = np.array([0.01] * self.AP_channels)
                tau = tf.constant(tau, dtype=tf.float32)
            ...
            # return input_batch_shape, F0_batch_shape, prev_phone_batch_shape, cur_phone_batch_shape, next_phone_batch_shape, phone_pos_batch_shape
                return loss
