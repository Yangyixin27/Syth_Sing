...
FILE_PATTERN = r'([a-z]+)_song([0-9]+)_([0-9]+)_([0-9]+)\.npy'########################################################
mfsc_path = "/Users/muyang/Desktop/EE599_Speech/2018_synth_sing_proj/Dataset/coldplay_data_npy/mfsc/"
# ap_path = "/Users/muyang/Desktop/EE599_Speech/2018_synth_sing_proj/Dataset/coldplay_data_npy/ap/"
f0_path = "/Users/muyang/Desktop/EE599_Speech/2018_synth_sing_proj/Dataset/coldplay_data_npy/f0coded/"
prev_phone_path = "/Users/muyang/Desktop/EE599_Speech/2018_synth_sing_proj/Dataset/coldplay_data_npy/phones/prev/"
cur_phone_path = "/Users/muyang/Desktop/EE599_Speech/2018_synth_sing_proj/Dataset/coldplay_data_npy/phones/current/"
next_phone_path = "/Users/muyang/Desktop/EE599_Speech/2018_synth_sing_proj/Dataset/coldplay_data_npy/phones/next/"
phone_pos_path = "/Users/muyang/Desktop/EE599_Speech/2018_synth_sing_proj/Dataset/coldplay_data_npy/phones/pos/"

singer2int = {"coldplay":1}
...
def load_acous_F0_phones(directory, sample_rate):
    ...
    for filename in randomized_files:
        ...
        if not ids:
            ...
            category_id = singer2int[ids[0][0]]
        ######################################################################################
        # audio, _ = librosa.load(filename, sr=sample_rate, mono=True)

        mfsc = np.load(mfsc_path + filename)
        # acous = acous.reshape(-1, 1)
        ...
        # F0 = F0.reshape(-1, 1)                                       ############################# add phonetic feats ###################################
        yield mfsc, F0, prev_phone, cur_phone, next_phone, phone_pos, filename, category_id


def trim_silence(audio, threshold, frame_length=2048):         ############################## this should be incoporated into preprocessing ####################
...
class AudioReader(object):
    ...
    def __init__(self, 
        ...
        self.acou_feat_dim = self.mfsc_dim      ###### non-control inputs to net, e.g. mfsc/ap/uv
    ...
    def thread_main(self, sess):
        ...
        while not stop:
            ...
            for mfsc, F0, prev_phone, cur_phone, next_phone, phone_pos, filename, category_id in iterator:
                print (filename)
                ...
                # print (self.acou_feat_dim)
                # acous = acous.reshape(-1, self.acou_feat_dim[0])    # inside the self.silence_threshold if statement  ########### the (60,) issue##############
                ...
                acous = np.pad(mfsc, [[self.receptive_field, 0], [0, 0]],'constant')    # after padding, shape of audio is [receptive_field + m, 60]
                ...
                if self.sample_size:
                    ...
                    while acous.shape[0] > self.receptive_field:
                        # print ("acous_shape:", acous.shape)
                        acous_piece = acous[:(self.receptive_field +     # don't need to worry about out of index error here. python does it automatically
                                        self.sample_size), :]      # here piece has shape [receptive_field + sample_size, 1] 
                        # sess.run(self.acous_enqueue,
                        #          feed_dict={self.acous_placeholder: acous_piece})

                        lc_piece = lc[:(self.receptive_field +     # don't need to worry about out of index error here. python does it automatically
                                        self.sample_size), :]      # here piece has shape [receptive_field + sample_size, 1]  
                        # print ("queue size:", sess.run(self.acous_queue.size()))
