...
"""
Directly predict the twelve parameters 
mu1, mu2, mu3, mu4 
...
def get_mixture_coef(output):
  ...
  return mu1, mu2, mu3, mu4, sigma1, sigma2, sigma3, sigma4, pi1, pi2, pi3, pi4
...
if __name__ == "__main__":
  ...
  NOUT = MFSC_dim * KMIX * 3 # pi, mu, simga
  # NOUT = MFSC_dim * 4
  NEPOCH = 2000
  ...
  prob = pi1 * d1.prob(y) + pi2 * d2.prob(y) + pi3 * d3.prob(y) + pi4 * d4.prob(y)
  prob = prob 
  logprob = -1.0 * tf.log(prob)
  result = tf.reduce_sum(logprob, axis = -1)
  loss = tf.reduce_mean(result, axis = -1)
  #######################################################
